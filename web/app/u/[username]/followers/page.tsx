...
export default function FollowersPage() {
  ...
  useEffect(() => {
    if (!loading && !me) {
      router.replace(`/login?next=${encodeURIComponent(`/u/${params.username}/followers`)}`);
    }
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="mx-auto flex w-full max-w-md flex-1 flex-col gap-4 px-6 py-10">
        <h1 className="text-2xl font-semibold tracking-tight">@{params.username}&apos;s followers</h1>
        {users === null ? (
